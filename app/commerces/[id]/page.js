...
export default function Page({ params }) {
    ...
    return (
        <div className="container-fluid h-100 text-center">
            ...
            <h1 className="mt-5 display-5">Reviews:</h1>
            { isLogged === 'true' ?
                <button className="btn btn-primary mt-1" data-bs-toggle="modal" data-bs-target="#exampleModal">Add review</button>
            :
                <p className="mt-1 fs-3">You must be logged in to add a review</p>
            }
            <div className="container-fluid mt-5 border rounded">
                {reviews.map((review, index) => (
                    <div key={index} className="row text-start">
                        <div className="col-10">
                            <p className="mt-1 fs-3">User: {review.name}</p>
                            <p className="mt-1 fs-3">Comment: {review.comment}</p>
                        </div>
            ...
            <div className="modal fade" id="exampleModal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content p-2">
                        <h4 className="text-center mt-1">Add review</h4>
                        ...
                        />
                        <label className="form-label mt-3" htmlFor="rating">
                            Rating
                        </label>
                        <input
                            className="form-control mb-3"
                        ...
                        <div className="d-flex justify-content-between">
                            ...
                            <button className="btn btn-primary m-1" onClick={() => handleModalSave(modalData.comment, modalData.rating)}>Save</button>
                        </div>
                    </div>
                </div>
            </div>
